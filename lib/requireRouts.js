...
module.exports = function(_s){
    ...
    _s.oReq.app.post('/login', function (req, res) {
        ...
        User.findOne(req.body.login).exec(function (err, user) {
            req.session.user = {};
            if(err) res.redirect('/');
            ...
            if(user)
            {
                ...
                var token = _s.oReq.jwt.sign({ userId : user.id, userName : user.userName }, _s.oConfig.session.secret, { algorithm: 'HS512'});
                console.log(token);
